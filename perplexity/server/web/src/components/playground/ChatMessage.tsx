...
function SourcesList({ sources }: SourcesListProps) {
  ...
  return (
    <div className="mt-3 pt-3 border-t border-gray-600">
      ...
      >
        <span className="text-neon-pink font-bold">[{isOpen ? '-' : '+'}]</span>
    ...
    >
      {copied ? (
        ...
        >
          ...
          <path
            fillRule="evenodd"
            d="M12.5 6a1 1 0 011 1v10.5a1 1 0 01-1 1h-9a1 1 0 01-1-1V7a1 1 0 011-1h9zM4 8.5V17h8V8.5H4z"
            clipRule="evenodd"
          />
        </svg>
      )}
...
function ChatMessageView({ message, isStreaming }: ChatMessageProps) {
  const isUser = message.role === 'user'
  const textContent = getTextContent(message.content)
  const isError =
    message.role === 'assistant' &&
    (Boolean(message.error) || textContent.startsWith('Error:'))
  const showSources =
    !isUser && !isError && message.sources && message.sources.length > 0

  return (
    <div
      className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-8 px-2`}
    >
              ...
              {isUser ? (
                <span className="whitespace-pre-wrap">
                  ...
                  {Array.isArray(message.content) &&
                    message.content.filter((p) => p.type === 'input_file')
                      .length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {message.content
                          ...
                          .map((p, i) => (
                            <span
                              key={i}
                              className="font-mono text-xs bg-gray-800 border border-gray-600 px-2 py-0.5 text-gray-400"
                            >
                              📎{' '}
                              {
                                (p as { type: 'input_file'; filename: string })
                                  .filename
                              }
                            </span>
                          ))}
                  ...
                  <div className="markdown-content">
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        td: ({ children }) => (
                          <td className="border-b border-gray-800 px-3 py-2 text-gray-400">
                            {children}
                          </td>
                        ),
                        // Horizontal rule
